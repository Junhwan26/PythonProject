...
class insert:
    def __init__(self,Parent):
        ...
        self.label_ip=Label(Parent,text="Server's IP : "+socket.gethostbyname(socket.getfqdn()))
        self.label_ip.pack()
...
root=Tk()
root.title("Server mangement")
root.geometry("300x480+100+100")
server_ip=Label(root, text = HOST)
server_ip.pack()

server_port=Label(root, text = PORT)
server_port.pack()

label1=ttk.Label(root,text='접속자 목록')
label1.pack()

listbox=Listbox(root, selectmode='extended',height=20)
listbox.pack()
...
class UserManager: # 사용자관리 및 채팅 메세지 전송을 담당하는 클래스
   ...
   def messageHandler(self, username, msg): # 전송한 msg를 처리하는 부분
      if msg[0] != '/': # 보낸 메세지의 첫문자가 '/'가 아니면
         now = time.localtime()
         self.sendMessageToAll('[%02d:%02d]%s:%s' %(now.tm_hour, now.tm_min,username, msg))
         print('[%02d:%02d:%02d]%s:%s' %(now.tm_hour, now.tm_min,now.tm_sec,username, msg))
         return
...
#b = Button(root, text = "kick user", command = delete(listbox))
#b.pack()
